...
const CardController = (props) => {
    const [newTitle, setNewTitle] = useState('My New Card ---'  + Math.floor(Math.random() * 100))
    const [newDescription, setNewDescription] = useState('Very interesting card description')
    ...
    const addCardHandler = () => {
        ...
        setNewTitle('My New Card ---' + Math.floor(Math.random() * 100));
        setNewDescription('Very interesting card description')
        setNewStatus('ToDo')
    };
...
export default connect(null, mapDispatchToProps)(CardController);